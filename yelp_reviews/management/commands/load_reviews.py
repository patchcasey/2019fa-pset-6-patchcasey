...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        # unsure if you need to do more here to make it work
    ...
    def handle(self, *args, **options):
        ...
        reviews1 = reviews0.dropna(subset=["review_id", "user_id"])[
            reviews0["review_id"].str.len() == 22
        ]
        reviews2 = reviews1.fillna(
            value={
                "funny": float(0),
                "cool": float(0),
                "useful": float(0),
        ...
        reviews3 = reviews2.astype(
            {"funny": "int64", "cool": "int64", "useful": "int64", "stars": "int64"}
        )
        ...
        with transaction.atomic():
            # the date is the index, so pull that out and assign to model
            date_objs = [DimDate(date=idx) for idx, analysis in df_dates.iteritems()]

            DimDate.objects.all().delete()
            DimDate.objects.bulk_create(date_objs)

        with transaction.atomic():
            # same as above but related to index are other facts to be added to model
            review_objs = [
                FactReview(
