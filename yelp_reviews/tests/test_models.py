...
class FactReviewBasicTest(DJTest):
    ...
    def create_FactReview(self, date=timezone.now(),count=1, stars=1, useful=1, funny=1, cool=1):
        x = DimDate.objects.create(date=date)
        return FactReview.objects.create(date=x, count=count,stars=stars,useful=useful,funny=funny,cool=cool)
